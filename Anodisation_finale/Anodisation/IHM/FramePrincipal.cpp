...
FramePrincipal::FramePrincipal( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	bSizerPanelNoteBook->Add( m_evt_panel_responsable, 1, wxALIGN_CENTER_VERTICAL|wxEXPAND, 5 );
	
	m_evt_panel_regleur=new EvtPanelRegleur(this);
	m_evt_panel_regleur->Hide();
	
	bSizerPanelNoteBook->Add( m_evt_panel_regleur, 1, wxALIGN_CENTER_VERTICAL|wxEXPAND, 5 );
	
	
	bSizerPrincipal->Add( bSizerPanelNoteBook, 1, wxEXPAND|wxALIGN_CENTER_HORIZONTAL, 5 );
